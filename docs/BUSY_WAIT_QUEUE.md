...
2. **User 2** calls DID while Venus busy:
   - Hears `consultant-busy` announcement → `crazytel-hold` music → repeats.
...
| File | Location |
|------|----------|
| Announcement | `asterisk/sounds/consultant_busy.wav` or `crazytel_calling_fe/public/sounds/consultant_busy.wav` |
